...
void render2DTree(Node* node, pcl::visualization::PCLVisualizer::Ptr& viewer, Box window, int& iteration, std::uint32_t depth = 0)
{
...
int main()
{
  ...
  KdTree<pcl::PointXYZ, 2>* tree = new KdTree<pcl::PointXYZ, 2>;
  ...
  //
  std::vector<std::vector<int>> clusters = euclideanCluster<pcl::PointXYZ, 2>(*cloud, tree, 3.0, 3, 3);
  //
